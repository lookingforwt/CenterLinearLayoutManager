    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (mAnchorInfo.mLayoutFromEnd) {
            ...
            if (mLayoutState.mAvailable > 0) {
                ...
                updateLayoutStateToFillStart(firstElement, startOffset, INVALID_OFFSET);
                mLayoutState.mExtra = extraForStart;
                fill(recycler, mLayoutState, state, false);
                startOffset = mLayoutState.mOffset;
            }
            ...
            if (mLayoutState.mAvailable > 0) {
                ...
                updateLayoutStateToFillEnd(lastElement, endOffset, INVALID_OFFSET);
                mLayoutState.mExtra = extraForEnd;
                fill(recycler, mLayoutState, state, false);
                endOffset = mLayoutState.mOffset;
            }
...
//        }
    ...
    private void layoutForPredictiveAnimations(RecyclerView.Recycler recycler,
        ...
        if (scrapExtraStart > 0) {
            ...
            updateLayoutStateToFillStart(getPosition(anchor), startOffset, INVALID_OFFSET);
        ...
        if (scrapExtraEnd > 0) {
            ...
            updateLayoutStateToFillEnd(getPosition(anchor), endOffset, INVALID_OFFSET);
    ...
    private void updateAnchorInfoForLayout(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        anchorInfo.assignCoordinateAsAlignCenter();
...
//                        : mOrientationHelper.getDecoratedStart(child);
                anchorInfo.mLayoutFromEnd = mOrientationHelper.getDecoratedStart(child) - mOrientationHelper.getStartAfterPadding() > mOrientationHelper.getEndAfterPadding() - mOrientationHelper.getDecoratedEnd(child);
                int rvCenter = (mOrientationHelper.getStartAfterPadding() + mOrientationHelper.getEndAfterPadding()) / 2;
                int childConsumed = mOrientationHelper.getDecoratedMeasurement(child);
                anchorInfo.mCoordinate = anchorInfo.mLayoutFromEnd ? rvCenter + childConsumed / 2 : rvCenter - childConsumed / 2;
        ...
        anchorInfo.mCoordinate = LayoutState.PENDING_OFFSET_ALIGN_CENTER;
        if (mShouldReverseLayout) {
            anchorInfo.mCoordinateOffset = -mPendingScrollPositionOffset;
        } else {
            anchorInfo.mCoordinateOffset = mPendingScrollPositionOffset;
        }
        return true;
    ...
    private void updateLayoutStateToFillEnd(AnchorInfo anchorInfo) {
        updateLayoutStateToFillEnd(anchorInfo.mPosition, anchorInfo.mCoordinate, anchorInfo.mCoordinateOffset);
    }

    private void updateLayoutStateToFillEnd(int itemPosition, int offset, int extOffset) {
        View child = findViewByPosition(itemPosition);
        if (child == null) {
            if (offset == LayoutState.PENDING_OFFSET_ALIGN_CENTER) {
                mLayoutState.mAvailable = (mOrientationHelper.getEndAfterPadding() - mOrientationHelper.getStartAfterPadding()) / 2;
            } else {
                mLayoutState.mAvailable = mOrientationHelper.getEndAfterPadding() - offset;
            }
            if (extOffset != INVALID_OFFSET) {
                mLayoutState.mAvailable -= extOffset; // 因为child已经被laid out, extOffset已经生效了, else分支不需要再处理
            }
            mLayoutState.mOffset = offset;
        } else {
            mLayoutState.mAvailable = mOrientationHelper.getEndAfterPadding() - mOrientationHelper.getDecoratedEnd(child);
    ...
    private void updateLayoutStateToFillStart(AnchorInfo anchorInfo) {
        updateLayoutStateToFillStart(anchorInfo.mPosition, anchorInfo.mCoordinate, anchorInfo.mCoordinateOffset);
    }

    private void updateLayoutStateToFillStart(int itemPosition, int offset, int extOffset) {
        View child = findViewByPosition(itemPosition);
        if (child == null) {
            if (offset == LayoutState.PENDING_OFFSET_ALIGN_CENTER) {
                mLayoutState.mAvailable = (mOrientationHelper.getEndAfterPadding() - mOrientationHelper.getStartAfterPadding()) / 2;
            ...
            mLayoutState.mAvailable = mOrientationHelper.getDecoratedStart(child) - mOrientationHelper.getStartAfterPadding();
    ...
    private void updateLayoutState(int layoutDirection, int requiredSpace,
        ...
        int scrollingOffset;
        int scrollingOffsetIfNoMore;
        int scrollingOffsetIfHasMore;
        if (layoutDirection == LayoutState.LAYOUT_END) {
            ...
            scrollingOffsetIfNoMore = (mOrientationHelper.getDecoratedEnd(child) + mOrientationHelper.getDecoratedStart(child)) / 2
                    - (mOrientationHelper.getEndAfterPadding() + mOrientationHelper.getStartAfterPadding()) / 2;
            scrollingOffsetIfHasMore = mOrientationHelper.getDecoratedEnd(child) - mOrientationHelper.getEndAfterPadding();
        } else {
            ...
            scrollingOffsetIfNoMore = -(mOrientationHelper.getDecoratedEnd(child) + mOrientationHelper.getDecoratedStart(child)) / 2
                    + (mOrientationHelper.getEndAfterPadding() + mOrientationHelper.getStartAfterPadding()) / 2;
            scrollingOffsetIfHasMore = -mOrientationHelper.getDecoratedStart(child) + mOrientationHelper.getStartAfterPadding();
        }
        boolean hasMore = mLayoutState.hasMore(state);
        if (hasMore) {
            scrollingOffset = scrollingOffsetIfHasMore;
        } else {
            scrollingOffset = scrollingOffsetIfNoMore;
        }
        mLayoutState.mAvailable = requiredSpace;
        if (canUseExistingSpace) {
            mLayoutState.mAvailable -= scrollingOffset; // 不管三七二十一mAvailable先全部吃掉scrollingOffset, 如果吃多了再fill中再吐出来
        }
    ...
    int fill(RecyclerView.Recycler recycler, LayoutState layoutState,
        ...
        if (layoutState.mScrollingOffset != LayoutState.SCROLLING_OFFSET_NaN) {
            // TODO ugly bug fix. should not happens
            if (layoutState.mAvailable < 0) {
                layoutState.mScrollingOffset += layoutState.mAvailable; // mAvailable将吃多的scrollingOffset吐出来还给mScrollingOffset
            }
        ...
        while ((layoutState.mInfinite || remainingSpace > 0) && layoutState.hasMore(state)) {
            ...
            if (layoutState.mOffset == LayoutState.PENDING_OFFSET_ALIGN_CENTER) {
                layoutState.mOffset = (mOrientationHelper.getEndAfterPadding() + mOrientationHelper.getStartAfterPadding()) / 2 + layoutChunkResult.mConsumed / 2 * layoutState.mLayoutDirection;
            } else {
    ...
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        if (mOrientation == VERTICAL) {
            ...
            if (layoutState.mOffset == LayoutState.PENDING_OFFSET_ALIGN_CENTER) {
                int center = (mOrientationHelper.getEndAfterPadding() + mOrientationHelper.getStartAfterPadding()) / 2;
                top = center - result.mConsumed / 2;
                bottom = top + result.mConsumed;
            } else if (layoutState.mLayoutDirection == LayoutState.LAYOUT_START) {
            ...
            if (layoutState.mOffset == LayoutState.PENDING_OFFSET_ALIGN_CENTER) {
                int center = (mOrientationHelper.getEndAfterPadding() + mOrientationHelper.getStartAfterPadding()) / 2;
                left = center - result.mConsumed / 2;
    ...
    static class LayoutState {
        ...
        static final int PENDING_OFFSET_ALIGN_CENTER = Integer.MIN_VALUE + 1; // TODO 会不会留下bug
         ...
         */
        int mOffset = PENDING_OFFSET_ALIGN_CENTER;
    ...
    static class AnchorInfo {
        ...
        int mPosition;
        int mCoordinate;
        int mCoordinateOffset; // TODO 新增字段是否多余
        ...
        void reset() {
            ...
            mCoordinateOffset = INVALID_OFFSET;
         ...
         * layoutFromEnd value
         */
        void assignCoordinateAsAlignCenter() {
            mCoordinate = LayoutState.PENDING_OFFSET_ALIGN_CENTER;
        }
